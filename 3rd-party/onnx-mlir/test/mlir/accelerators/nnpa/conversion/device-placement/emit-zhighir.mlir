...
module {
  func.func @mnist(%arg0: tensor<1x1x28x28xf32>) -> tensor<1x10xf32> {
    ...
    %8 = "onnx.Reshape"(%2, %6) {allowzero = 0 : si64} : (tensor<16x4x4x10xf32>, tensor<2xi64>) -> tensor<256x10xf32>
    ...
    %10 = "onnx.Relu"(%9) {device = "nnpa"} : (tensor<1x8x28x28xf32>) -> tensor<1x8x28x28xf32>
    %11 = "onnx.MaxPoolSingleOut"(%10) {auto_pad = "NOTSET", ceil_mode = 0 : si64, device = "nnpa", kernel_shape = [2, 2], pads = [0, 0, 0, 0], storage_order = 0 : si64, strides = [2, 2]} : (tensor<1x8x28x28xf32>) -> tensor<1x8x14x14xf32>
    ...
    %13 = "onnx.Relu"(%12) {device = "nnpa"} : (tensor<1x16x14x14xf32>) -> tensor<1x16x14x14xf32>
    %14 = "onnx.MaxPoolSingleOut"(%13) {auto_pad = "NOTSET", ceil_mode = 0 : si64, device = "nnpa", kernel_shape = [3, 3], pads = [0, 0, 0, 0], storage_order = 0 : si64, strides = [3, 3]} : (tensor<1x16x14x14xf32>) -> tensor<1x16x4x4xf32>
    %15 = "onnx.Reshape"(%14, %5) {allowzero = 0 : si64} : (tensor<1x16x4x4xf32>, tensor<2xi64>) -> tensor<1x256xf32>
...
// CHECK-DAG:       [[VAR_6_:%.+]] = "onnx.Reshape"([[VAR_0_]], [[VAR_4_]]) <{allowzero = 0 : si64}> {onnx_node_name = "onnx.Reshape_0"} : (tensor<16x4x4x10xf32>, tensor<2xi64>) -> tensor<256x10xf32>
...
// CHECK:           [[VAR_19_:%.+]] = "onnx.Reshape"([[VAR_18_]], [[VAR_3_]]) <{allowzero = 0 : si64}> {onnx_node_name = "onnx.Reshape_7"} : (tensor<1x16x4x4xf32>, tensor<2xi64>) -> tensor<1x256xf32>
// CHECK:           [[VAR_20_:%.+]] = "onnx.Gemm"([[VAR_19_]], [[VAR_6_]], [[VAR_5_]]) <{alpha = 1.000000e+00 : f32, beta = 1.000000e+00 : f32, transA = 0 : si64, transB = 0 : si64}> {device = "cpu", onnx_node_name = "onnx.Gemm_8"} : (tensor<1x256xf32>, tensor<256x10xf32>, tensor<1x10xf32>) -> tensor<1x10xf32>
