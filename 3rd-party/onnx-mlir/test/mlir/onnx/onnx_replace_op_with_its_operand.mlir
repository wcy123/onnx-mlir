// RUN: onnx-mlir-opt --replace-op-with-its-operand="list-id-regex=0:/model/layers.*/mask" --canonicalize %s -split-input-file | FileCheck %s
// RUN: onnx-mlir --replace-op-with-its-operand="0:/model/layers.*/mask" --EmitONNXIR --printIR %s | FileCheck %s --check-prefix=DRIVER
...
func.func @test_ignore_attention_layer(%arg0: tensor<1x?xi64>, %arg1: tensor<1x?xi64>, %arg2: tensor<1x12x?x64xf32>, %arg3: tensor<1x12x64x?xf32>) -> tensor<1x12x?x?xf32>
{
  ...
  %14 = "onnx.Add"(%13, %12) { onnx_node_name = "/model/layers.20/mask" } : (tensor<1x12x?x?xf32>, tensor<1x1x?x?xf32>) -> tensor<1x12x?x?xf32>
...
// DRIVER-LABEL:  func.func @test_ignore_attention_layer
// DRIVER-SAME:   ([[PARAM_0_:%.+]]: tensor<1x?xi64>, [[PARAM_1_:%.+]]: tensor<1x?xi64>, [[PARAM_2_:%.+]]: tensor<1x12x?x64xf32>, [[PARAM_3_:%.+]]: tensor<1x12x64x?xf32>) -> tensor<1x12x?x?xf32> {
// DRIVER:           [[VAR_0_:%.+]] = "onnx.MatMul"([[PARAM_2_]], [[PARAM_3_]]) {onnx_node_name = "onnx.MatMul_0"} : (tensor<1x12x?x64xf32>, tensor<1x12x64x?xf32>) -> tensor<1x12x?x?xf32>
// DRIVER:           [[VAR_1_:%.+]] = "onnx.Softmax"([[VAR_0_]]) <{axis = -1 : si64}> {onnx_node_name = "onnx.Softmax_1"} : (tensor<1x12x?x?xf32>) -> tensor<1x12x?x?xf32>
