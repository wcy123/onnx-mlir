...
└──────────────┘     └──────────────┘     └──────────────┘     └──────────────┘
   ...
   arith.constant       hip operations      wrapper functions      (init/compute/
...
**Key characteristics:**
- High-level ONNX operations (`onnx.Conv`, `onnx.Relu`)
- Tensor types with compile-time shapes
- Inline constants via `arith.constant`
...
| **Constants** | Inline `arith.constant` | `llvm.mlir.global` | `.data` section | GPU memory |
...
```
Stage 1 (ONNX-MLIR):    arith.constant dense<[...]> : tensor<...>
                        └─ Inlined in @main function
                              ↓
