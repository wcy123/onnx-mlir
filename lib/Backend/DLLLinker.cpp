...
// NOTE: LLD is optional - only needed for Native mode compilation
#ifdef HAVE_LLD
namespace lld {
namespace coff {
...
{
#ifdef _WIN32
    return linkDLL_Windows(objectFile, outputDLL, libraries, libraryPaths, exportSymbols);
#else
...
bool DLLLinker::linkDLL_Windows(
    const std::string& objectFile,
    const std::string& outputDLL,
    const std::vector<std::string>& libraries,
    const std::vector<std::string>& libraryPaths,
    const std::vector<std::string>& exportSymbols)
{
    ...
    return true;
#else
    std::cerr << "ERROR: LLD library not available. Native mode requires LLVM built with LLD.\n";
    std::cerr << "       Rebuild LLVM with: -DLLVM_ENABLE_PROJECTS=\"mlir;lld\"\n";
