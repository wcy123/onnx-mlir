...
int main() {
    ...
    // Prepare
    result = runtime_prepare_inference(state, &multi_inputs_span, &multi_outputs_span, &inf_data);
    assert(result == 0);
    ...
    // Prepare
    result = runtime_prepare_inference(state, &var_rank_inputs_span, &var_outputs_span, &inf_data);
    assert(result == 0);
