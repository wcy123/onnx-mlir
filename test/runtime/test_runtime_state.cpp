...
int main() {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        result = runtime_state_init(&state2);
        assert(result == 0);
        assert(state2 != nullptr);

        result = runtime_state_cleanup(state2);
        assert(result == 0);
    }
    ...
    std::cout << "  - hipStreamCreate, miopenCreate, miopenSetStream, hipblasLtCreate\n";
    std::cout << "  - hipStreamSynchronize, hipblasLtDestroy, miopenDestroy, hipStreamDestroy\n";
